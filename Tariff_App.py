...
@st.cache_data
def load_data():
    url = 'https://drive.google.com/uc?export=download&id=1OvJqH7kMdmqeK6ldDnRNLs-9U3QhhDDl'
    try:
        df = pd.read_csv(url)
...
st.sidebar.expander("Debug Info", expanded=False).write({
    "Data loaded": "Yes" if 'df' in locals() and not df.empty else "No",
    "DataFrame shape": df.shape if 'df' in locals() and not df.empty else None,
    "DataFrame columns": df.columns.tolist() if 'df' in locals() and not df.empty else None,
    "Agreement columns": [col for col in df.columns if '_ad_val_rate' in col] if 'df' in locals() and not df.empty else None
})
